...
def worker(proc_num, queue, iter):
    while True:
        ...
        polarities = evaluate_methods.run_method(
            positive_seeds,
            negative_seeds,
            test_embed,
        ...
        util.write_pickle(
            polarities,
            constants.POLARITIES + year + "-synth-adj-coha-" + str(iter) + ".pkl",
        )
...
def make_synthetic_data(
    ...
    for i in xrange(num_synth):
        amel_vecs.append(
            _sample_vec2(new_pos, old_neg, counts, new_weight, seed=i + seed_offset)
        )
...
def _sample_vec(pos_words, neg_words, counts, pos_weight, seed):
    ...
    for i, word in enumerate(pos_words):
        ...
        sample_vec = (
            pos_weights[i]
            * pos_weight
            * np.random.multinomial(sample_size, sample_vec.todense().A[0])
        )
    ...
    for neigh in vec.nonzero()[1]:
        val = max(
            np.log(new_mat[-1, neigh] / (synth_prob * new_mat[neigh, :].sum() ** 0.75)),
            0,
        )
        if np.isfinite(val):
            vec[0, neigh] = val
