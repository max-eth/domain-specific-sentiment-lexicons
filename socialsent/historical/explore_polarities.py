...
def build_boot_timeseries(suffix="-test", years=constants.YEARS):
    ...
    for year in years:
        polarities_list = util.load_pickle(
            constants.POLARITIES + year + suffix + ".pkl"
        )
...
def zscore(dictseries, years=range(1850, 2000, 10)):
    ...
    for year in years:
        ...
        for word in yearseries[year]:
            timeseries[word].append((dictseries[word][year] - year_mean) / year_std)
    return timeseries
...
def get_yearseries(dictseries, years=range(1850, 2000, 10)):
    ...
    for year in years:
        year_series = {
            word: dictseries[word][year]
            for word in dictseries
            if not np.isnan(dictseries[word][year])
        }
