...
def pmi(count_embeds, positive_seeds, negative_seeds, smooth=0.01, **kwargs):
    ...
    for w in count_embeds.iw:
        if w not in positive_seeds and w not in negative_seeds:
            pol = sum(
                np.log(counts[w_index[w], c_index[seed]] + smooth)
                - np.log(counts[w_index[seed], :].sum())
...
def graph_propagate(embeddings, positive_seeds, negative_seeds, **kwargs):
    ...
    pos_alpha = run_graph_propagate(
        [embeddings.wi[seed] for seed in positive_seeds], pos_alpha, M, **kwargs
    )
    pos_alpha = pos_alpha + pos_alpha.T
    print("Getting negative scores..")
    neg_alpha = run_graph_propagate(
        [embeddings.wi[seed] for seed in negative_seeds], neg_alpha, M, **kwargs
    )
    ...
    for w in util.logged_loop(index):
        if w not in positive_seeds and w not in negative_seeds:
            pos_pols[w] = sum(
                pos_alpha[index[w], index[seed]]
                for seed in positive_seeds
                if seed in index
            )
            neg_pols[w] = sum(
                neg_alpha[index[w], index[seed]]
                for seed in negative_seeds
                if seed in index
            )
...
def _bootstrap_func(
    embeddings, positive_seeds, negative_seeds, boot_size, score_method, seed, **kwargs
):
    np.random.seed(seed)
    ...
    return {
        word: score
        for word, score in polarities.items()
