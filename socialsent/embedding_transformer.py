...
class SimpleSGD(Optimizer):
    def __init__(self, lr=5, momentum=0.0, decay=0.0, nesterov=False, **kwargs):
...
class OthogonalRegularizer(Regularizer):
    ...
    def __call__(self, loss):
        loss += (
            K.sum(K.square(self.p.dot(self.p.T) - T.identity_like(self.p)))
            * self.strength
        )
        return loss

    def get_config(self):
        return {"name": self.__class__.__name__, "strength": self.strength}
...
class DatasetMinibatchIterator:
    def __init__(
        self, embeddings, positive_seeds, negative_seeds, batch_size=512, **kwargs
    ):
    ...
    def shuffle(self):
        ...
        self.e1, self.e2, self.y, self.words = (
            ...
            self.e2[perm],
            self.y[perm],
            [self.words[i] for i in perm],
        )

    def __iter__(self):
        for i in range(self.n_batches):
            batch = np.arange(
                i * self.batch_size, min(self.y.size, (i + 1) * self.batch_size)
            )
            yield {
...
def get_model(
    inputdim, outputdim, regularization_strength=0.01, lr=0.000, cosine=False, **kwargs
):
    transformation = Dense(inputdim, init="identity", W_constraint=Orthogonal())
    ...
    model.add_node(
        Lambda(lambda x: x[:, :outputdim]), input="transformed1", name="projected1"
    )
    model.add_node(
        Lambda(lambda x: -x[:, :outputdim]), input="transformed2", name="negprojected2"
    )

    if cosine:
        model.add_node(
            Lambda(
                lambda x: x / K.reshape(K.sqrt(K.sum(x * x, axis=1)), (x.shape[0], 1))
            ),
            name="normalized1",
            input="projected1",
        )
        ...
        model.add_node(
            Lambda(lambda x: K.reshape(K.sqrt(K.sum(x * x, axis=1)), (x.shape[0], 1))),
            name="distances",
...
def apply_embedding_transformation(
    embeddings,
    positive_seeds,
    negative_seeds,
    n_epochs=5,
    ...
    **kwargs
    ...
    dataset = DatasetMinibatchIterator(
        embeddings, positive_seeds, negative_seeds, **kwargs
    )
