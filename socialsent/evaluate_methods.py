...
def evaluate_overlap_methods():
    ...
    hist_counts = create_representation(
        "Explicit", constants.COHA_COUNTS + "2000", normalize=False
    )
    hist_words = set(hist_embed.iw)
    eval_words = eval_words.intersection(hist_words)
...
def evaluate_adj_methods():
    ...
    hist_embed = create_representation("SVD", constants.COHA_EMBEDDINGS + "2000")
    hist_counts = create_representation(
        "Explicit", constants.COUNTS + "1990", normalize=False
    )
    hist_words = set(hist_embed.iw)
    eval_words = eval_words.intersection(hist_words)
...
def evaluate_finance_methods():
    ...
    gi_neut = np.random.choice(
        gi_neut, int((float(len(gi_neut)) / (len(gi) - len(gi_neut)) * len(lexicon)))
    )
    for word in gi_neut:
        lexicon[word] = 0
...
def run_method(
    ...
    **kwargs
    ...
    if transform_embeddings:
        ...
        embeddings = embedding_transformer.apply_embedding_transformation(
            embeddings, positive_seeds, negative_seeds, n_dim=50
        )
    if post_densify:
        polarities = method(embeddings, positive_seeds, negative_seeds, **kwargs)
        top_pos = [
            word for word in sorted(polarities, key=lambda w: -polarities[w])[:150]
        ]
        top_neg = [
            word for word in sorted(polarities, key=lambda w: polarities[w])[:150]
        ]
...
def print_polarities(polarities, lexicon):
    for w, p in sorted(polarities.items(), key=itemgetter(1), reverse=True):
        print (util.GREEN if lexicon[w] == 1 else util.RED) + "{:}: {:0.5f}".format(
            w, p
        ) + util.ENDC
...
def evaluate(polarities, lexicon, eval_words, tau_lexicon=None, tern=True):
    ...
    if not tern:
        return
    tau, cmn_f1, maj_f1, conf_mat = ternary_metrics(
        polarities, lexicon, eval_words, tau_lexicon=tau_lexicon
    )
    ...
    print
    if tau:
        print "Latex table line: {:2.1f} & {:2.1f} & {:.2f}\\\\".format(
            100 * auc, 100 * cmn_f1, tau
        )
    else:
...
def binary_metrics(
    polarities, lexicon, eval_words, print_predictions=False, top_perc=None
):
    eval_words = [word for word in eval_words if lexicon[word] != 0]
    y_prob, y_true = [], []
    if top_perc:
        polarities = {
            word: polarities[word]
            for word in sorted(
                eval_words, key=lambda w: abs(polarities[w] - 0.5), reverse=True
    ...
    best_accuracy = max(
        [(1 + i - cumsum[i] + positive - cumsum[i]) / float(n) for i in range(n)]
    )
...
def ternary_metrics(polarities, lexicon, eval_words, tau_lexicon=None):
    ...
    cmn_labels = [
        1 if val >= pos_thresh else -1 if val <= neg_thresh else 0 for val in y_prob
    ]
    if not tau_lexicon == None:
        tau = kendalltau(
            *zip(*[(polarities[word], tau_lexicon[word]) for word in kendall_words])
        )[0]
    else:
        tau = None
    maj_f1 = f1_score(
        y_true, np.repeat(sp.stats.mode(y_true)[0][0], len(y_true)), average="macro"
    )
    cmn_f1 = f1_score(y_true, cmn_labels, average="macro")
    label_func = (
        lambda entry: 1 if entry > pos_thresh else -1 if entry < neg_thresh else 0
    )
    conf_mat = confusion_matrix(y_true, [label_func(entry) for entry in y_prob])
    return tau, cmn_f1, maj_f1, conf_mat


def optimal_tern_acc(
    polarities, lexicon, eval_words, threshes=np.arange(0.95, 0.0, -0.01)
):
    """
