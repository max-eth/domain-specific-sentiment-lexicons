...
def wordnet_similarity_matrix(embeddings):
    ...
    for i, word in enumerate(words):
        ...
        synonyms = set(
            chain.from_iterable(
                [o_word.lemma_names() for o_word in wn.synsets(words[word])]
            )
        )
        for o_word in synonyms:
            if o_word in lemmas:
                sim_mat[embeddings.wi[word], embeddings.wi[lemmas[o_word]]] = 1.0
...
def transition_matrix(
    embeddings, word_net=False, first_order=False, sym=False, trans=False, **kwargs
):
    """
