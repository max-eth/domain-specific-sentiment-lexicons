...
def make_kuperman_scores_lexicon():
    ...
    for i, line in enumerate(
        util.lines(constants.LEXICONS + "kuperman/raw_ratings.csv")
    ):
        if i == 0:
            continue
        info = line.split(",")
...
def make_140_scores_lexicon():
    polarities = {}
    for line in util.lines(
        constants.LEXICONS + "Sentiment140-Lexicon-v0.1/unigrams-pmilexicon.txt"
    ):
        info = line.split()
...
def make_qwn_scores_lexicon():
    ...
    for line in util.lines(
        constants.LEXICONS + "qwn/turneyLittman_propSyn_08_mcr30-noAntGloss.dict"
    ):
        info = line.split("\t")
        mod = float(info[3])
    ...
    for line in util.lines(
        ...
        for word in info[2].split(", "):
            if not "_" in word:
                polarities[word.split("#")[0]] += mod
    polarities = {
        word: np.sign(val) for word, val in polarities.iteritems() if val != 0
    }
...
def make_bingliu_lexicon():
    ...
    for polarity in ["positive", "negative"]:
        for line in util.lines(
            constants.LEXICONS + "bl_opinion_lexicon/{:}-words.txt".format(polarity)
        ):
            try:
...
def load_lexicon(name=constants.LEXICON, remove_neutral=True):
    ...
    return (
        {w: p for w, p in lexicon.iteritems() if p != 0} if remove_neutral else lexicon
    )
...
def compare_lexicons(print_disagreements=False):
    ...
    for l1, l2 in itertools.combinations(lexicons.keys(), 2):
        ...
        print(
            ...
            "agreement: {:.2f}".format(
                100.0
                * sum(1 if ps1[w] == ps2[w] else 0 for w in common_words)
                / len(common_words)
            ),
        )
        common_words = set([word for word in ps1.keys() if ps1[word] != 0]) & set(
            [word for word in ps2.keys() if ps2[word] != 0]
        )
        print(
            l1,
            l2,
            "agreement ignoring neutral: {:.2f}".format(
                100.0
                * sum(1 if ps1[w] * ps2[w] == 1 else 0 for w in common_words)
                / len(common_words)
            ),
        )
